    ...
    try {
        ...
        if (!result.data || result.data.length === 0) {
            ...
            document.getElementById("store-grid").innerHTML = `
                <div class="col-span-full py-12 text-center text-slate-400">
                    ${window.__('operations_ui.store.no_merchandise')}
                </div>`;
            return;
        }
        ...
        document.getElementById("store-grid").innerHTML = result.data
            ...
            <div class="glass-card rounded-2xl p-6 space-y-4 hover:shadow-xl transition-all border-b-4 ${item.stock_quantity > 5 ? "border-primary-500" : "border-amber-500"}">
                <div class="flex justify-between items-start">
                    <div>
                        <span class="text-[10px] font-black uppercase tracking-widest text-primary-500">
                            ${item.category || window.__('operations_ui.store.product_fallback')}
                        </span>
                ...
                <div class="flex justify-between items-center text-sm">
                    <span class="text-slate-500">${window.__('operations_ui.store.in_stock_prefix')}</span>
                ...
                <button
                    ...
                    ${item.stock_quantity <= 0 ? window.__('operations_ui.store.out_of_stock') : window.__('operations_ui.store.quick_sale')}
                </button>
            </div>`,
            )
            .join("");
        ...
        document.getElementById("store-grid").innerHTML = `
            <div class="col-span-full py-12 text-center space-y-3">
                <p class="text-rose-500 font-bold">${window.__('operations_ui.store.load_failed')}</p>
                <p class="text-sm text-slate-400">${e.message}</p>
                <button onclick="window.renderStore()" class="text-xs text-primary-600 underline">${window.__('operations_ui.clients.try_again')}</button>
            </div>`;
        OperationsUI.toast(window.__('operations_ui.store.load_store_failed'), "error");
    }
}
    ...
    return `
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 p-4 bg-slate-50 dark:bg-slate-900">
            <div class="flex justify-between items-center mb-3">
                <span class="text-sm font-medium text-slate-500">${window.__('operations_ui.store.price_per_unit')}</span>
                <span class="font-bold">${currencyLabel}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-sm font-medium text-slate-500">${window.__('operations_ui.store.quantity_label')}</span>
                <span class="font-bold">${quantity}</span>
            </div>
            <div class="border-t border-slate-200 dark:border-slate-800 mt-4 pt-4 flex justify-between items-center">
                <span class="text-sm font-medium text-slate-500">${window.__('operations_ui.store.total_label')}</span>
...
export async function showQuickSale(itemId) {
    ...
    if (!item) {
        OperationsUI.toast(
            window.__('operations_ui.store.item_not_found'),
            "error",
        );
        return;
    }
    ...
    const existingTab = () => `
        <div id="existing-customer-form" class="space-y-4">
            <div class="space-y-2">
                <label class="text-sm font-bold text-slate-600 dark:text-slate-400">${window.__('operations_ui.store.select_customer')}</label>
                <input type="hidden" id="sale-customer-id" value="">
                <div class="relative">
                    <input type="text" id="sale-customer-search" placeholder="${window.__('operations_ui.store.customer_search_placeholder')}"
                           autocomplete="off"
    ...
    const walkInTab = () => `
        <div id="walkin-customer-form" class="space-y-4 hidden">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-1">
                    <label class="text-xs font-bold text-slate-500 uppercase">${window.__('operations_ui.store.full_name_label')} <span class="text-rose-500">*</span></label>
                    <input type="text" id="walkin-fullname" placeholder="${window.__('operations_ui.store.full_name_placeholder')}"
            ...
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-500 uppercase">${window.__('operations_ui.store.email_label')} <span class="text-slate-400">${window.__('operations_ui.store.email_optional')}</span></label>
        ...
        <div class="space-y-6">
            ...
            <div class="flex rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 text-sm font-bold">
                <button id="btn-existing" onclick="window.toggleSaleMode('existing')"
                    ...
                    ${window.__('operations_ui.store.existing_client')}
                </button>
                <button id="btn-walkin" onclick="window.toggleSaleMode('walkin')"
                    ...
                    ${window.__('operations_ui.store.walkin_label')}
                </button>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-600 dark:text-slate-400">${window.__('operations_ui.store.currency_label')}</label>
                ...
                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-600 dark:text-slate-400">${window.__('operations_ui.store.quantity_label')}</label>
            ...
            <button id="sale-submit-btn" onclick="window.submitQuickSale()"
                ...
                ${window.__('operations_ui.store.confirm_purchase')}
            </button>
        </div>`;

    OperationsUI.openModal(`${window.__('operations_ui.store.sale_prefix')}${item.name}`, content);
    ...
    if (saleClientPicker.error) {
        html += `
            <div class="px-4 py-3 space-y-1">
                <p class="text-sm text-rose-500 font-bold">${window.__('operations_ui.store.load_clients_failed')}</p>
                <p class="text-xs text-slate-400">${_esc(saleClientPicker.error)}</p>
                <button onclick="window.retrySaleClients()"
                        class="text-xs text-primary-600 underline">${window.__('operations_ui.clients.try_again')}</button>
            </div>`;
        ...
        html += '<p class="px-4 py-3 text-xs text-slate-400 italic">' + window.__('operations_ui.store.no_clients_found') + '</p>';
    }
...
export async function submitQuickSale() {
    ...
    try {
        if (!currencyId) {
            OperationsUI.toast(window.__('operations_ui.store.select_currency'), "warning");
            return;
        }

        if (!Number.isInteger(quantity) || quantity < 1) {
            OperationsUI.toast(
                window.__('operations_ui.store.quantity_invalid'),
                "warning",
            );
            return;
        }
        ...
        if (isWalkIn) {
            ...
            if (!fullname || !phone) {
                OperationsUI.toast(
                    window.__('operations_ui.store.walkin_required_fields'),
                    "warning",
                );
                return;
            }
            ...
            if (!customerId) {
                OperationsUI.toast(window.__('operations_ui.store.select_customer_required'), "warning");
                return;
            }
        ...
        OperationsUI.toast(window.__('operations_ui.store.sale_success'), "success");
        ...
        if (btn) {
            btn.disabled = false;
            btn.textContent = window.__('operations_ui.store.confirm_purchase');
        }
    }
}
