// public/js/operations/clients.js

let clientSearchTimeout = null;
let activeClientFilter  = '';

function applyClientFilter(pillEl) {
...
async function showClientDetails(userId) {
    ...
    try {
        ...
        let actionButton = '';
        if (user.has_frozen_booking && user.frozen_booking_id) {
            actionButton = `
                <button onclick="handleUnfreeze(${user.frozen_booking_id}, ${user.id})"
                    class="bg-sky-500 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:scale-105 transition-all btn-single-action">
                    ❄ Unfreeze Package
                </button>`;
        } else if (!user.active_package) {
            actionButton = `
                ...
                ${packages.map(p => `
                    <button onclick="handlePackageAssign(${userId}, ${p.id})"
                        ...
                        <span class="text-sm text-slate-500">
                            ...
                            ${p.validity_days != null ? p.validity_days + ' Days' : 'No expiry'}
                        </span>
