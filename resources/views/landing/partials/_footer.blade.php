...
<footer class="bg-primary-900 dark:bg-black text-primary-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-12">
            <div class="sm:col-span-2 lg:col-span-1">
                ...
                <p class="text-sm text-primary-300 leading-relaxed mb-6">{{ $s->siteTagline }}</p>
                ...
                <div class="flex gap-3">
                    @foreach($socialEntries as $entry)
                        <a href="{{ $entry->url ?? '#' }}" target="_blank" rel="noopener" class="w-10 h-10 rounded-xl bg-primary-800 hover:bg-primary-600 flex items-center justify-center transition-colors">
            ...
            <div>
                ...
                <ul class="space-y-3">
                    ...
                    <li><a href="{{ route('static-pages.show', $page->slug) }}" class="text-sm text-primary-300 hover:text-primary-200 transition-colors">{{ $page->getTranslation('title', app()->getLocale()) }}</a></li>
                    @empty
                    <li><span class="text-sm text-primary-500">-</span></li>
                    @endforelse
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-4">
                    <li class="flex items-start gap-3">
                        ...
                        <span class="text-sm text-primary-200">{{ $s->contactAddress }}</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i data-lucide="phone" class="w-5 h-5 text-primary-400 flex-shrink-0"></i>
                        <span class="text-sm text-primary-200">{{ $s->contactPhone }}</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i data-lucide="mail" class="w-5 h-5 text-primary-400 flex-shrink-0"></i>
                        <span class="text-sm text-primary-200">{{ $s->contactEmail }}</span>
                    </li>
                    <li class="flex items-center gap-3">
                        <i data-lucide="clock" class="w-5 h-5 text-primary-400 flex-shrink-0"></i>
                        <span class="text-sm text-primary-200">{{ $s->openingHoursWeekdays }}<br>{{ $s->openingHoursWeekends }}</span>
                    </li>
                </ul>
            </div>
        </div>

        <div class="py-6 border-t border-primary-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p class="text-sm text-primary-400">© {{ date('Y') }} {{ $s->footerCopyright }}. {{ __('landing.all_rights_reserved') }}</p>

            <div class="flex items-center gap-2">
                <i data-lucide="globe" class="w-4 h-4 text-primary-400"></i>
                <select class="bg-transparent text-sm text-primary-300 border-none focus:ring-0 cursor-pointer hover:text-primary-200 transition-colors" onchange="window.location.href='{{ url('locale') }}/' + this.value">
