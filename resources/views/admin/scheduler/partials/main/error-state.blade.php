...
<div id="error-state" class="hidden flex flex-col items-center justify-center py-24 gap-4 text-center">
    ...
    <div>
        <h3 id="error-title" class="text-lg font-bold text-gray-900 dark:text-white">{{ __('dashboard.scheduler_ui.error_state.title') }}</h3>
        <p id="error-message" class="text-sm text-gray-500 dark:text-gray-400 mt-1 max-w-xs">{{ __('dashboard.scheduler_ui.error_state.description') }}</p>
    </div>
    <button id="btn-retry"
        class="px-4 py-2 bg-primary-600 text-white rounded-lg text-sm font-semibold hover:bg-primary-500 transition-colors">
        {{ __('dashboard.scheduler_ui.error_state.retry') }}
    </button>
</div>