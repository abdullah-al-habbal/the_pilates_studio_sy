...
<div id="tab-attendees" class="hidden space-y-4">
    <div id="capacity-bar-wrap"
        ...
        <div
            ...
            <span>{{ __('dashboard.scheduler_ui.modal.live_engagement') }}</span>
            <span class="text-primary-600 dark:text-primary-400"><span id="capacity-reserved">0</span> /
                <span id="capacity-total">0</span> {{ __('dashboard.scheduler_ui.modal.reserved') }}</span>
        </div>
    ...
    <div id="attendees-error" class="hidden flex flex-col items-center py-16 gap-4 text-red-400">
        ...
        <p class="text-sm font-bold text-red-500">{{ __('dashboard.scheduler_ui.modal.unable_to_load_reservations') }}</p>
        <p id="attendees-error-message" class="text-xs text-red-300">{{ __('dashboard.scheduler_ui.modal.fetch_error') }}</p>
    </div>
    ...
    <div id="attendees-empty" class="hidden flex flex-col items-center py-16 gap-4 text-gray-400">
        ...
        <p class="text-sm font-bold text-gray-500">{{ __('dashboard.scheduler_ui.modal.no_reservations_title') }}</p>
        <p class="text-xs text-gray-400">{{ __('dashboard.scheduler_ui.modal.no_reservations_desc') }}</p>
    </div>
