...
<div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <div class="space-y-1">
        <h2 class="text-2xl font-bold tracking-tight">{{ __('dashboard::dashboard.operations_ui.notifications.title') }}</h2>
        <p class="text-slate-500">{{ __('dashboard::dashboard.operations_ui.notifications.description') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass-card rounded-2xl p-6 space-y-5">
            <h3 class="text-lg font-bold flex items-center gap-2">
                <span class="text-2xl">🔔</span> {{ __('dashboard::dashboard.operations_ui.notifications.compose_title') }}
            </h3>

            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-500 uppercase">{{ __('dashboard::dashboard.notifications.field_title') }}</label>
                <input type="text" id="notif-title" maxlength="255" placeholder="{{ __('dashboard::dashboard.operations_ui.notifications.title_placeholder') }}"
            ...
            <div class="space-y-1">
                <label class="text-xs font-bold text-slate-500 uppercase">{{ __('dashboard::dashboard.operations_ui.notifications.field_body') }}</label>
                <textarea id="notif-body" rows="4" maxlength="1000"
                    placeholder="{{ __('dashboard::dashboard.operations_ui.notifications.body_placeholder') }}"
                    class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 rounded-xl border-transparent
            ...
            {{-- Target --}}
            <div class="space-y-2">
                <label class="text-xs font-bold text-slate-500 uppercase">{{ __('dashboard::dashboard.operations_ui.notifications.target_audience') }}</label>
                <div class="flex gap-3">
                    <label class="flex-1 flex items-center gap-3 p-3 rounded-xl cursor-pointer border-2 border-transparent
                        ...
                        <div>
                            <p class="text-sm font-bold">{{ __('dashboard::dashboard.operations_ui.notifications.all_users') }}</p>
                            <p class="text-xs text-slate-400">{{ __('dashboard::dashboard.operations_ui.notifications.all_users_desc') }}</p>
                        </div>
                    </label>
                    <label class="flex-1 flex items-center gap-3 p-3 rounded-xl cursor-pointer border-2 border-transparent
                                  has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50 dark:has-[:checked]:bg-primary-900/20
                        ...
                        <div>
                            <p class="text-sm font-bold">{{ __('dashboard::dashboard.operations_ui.notifications.specific_users') }}</p>
                            <p class="text-xs text-slate-400">{{ __('dashboard::dashboard.operations_ui.notifications.specific_users_desc') }}</p>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div id="notif-user-picker" class="space-y-3 hidden">
                <div class="relative">
                    <input type="text" id="notif-user-search" placeholder="{{ __('dashboard::dashboard.operations_ui.notifications.user_search_placeholder') }}"
                ...
                <div id="notif-selected-users" class="flex flex-wrap gap-2 min-h-[2rem]">
                    <p class="text-xs text-slate-400 italic">{{ __('dashboard::dashboard.operations_ui.notifications.no_users_selected') }}</p>
                </div>
            </div>
            ...
            <button id="notif-send-btn" onclick="OperationsNotifications.send()"
                ...
                {{ __('dashboard::dashboard.operations_ui.notifications.send_button') }}
            </button>
        </div>
        ...
        <div class="glass-card rounded-2xl p-6 space-y-4">
            <h3 class="text-lg font-bold flex items-center gap-2">
                <span class="text-2xl">📊</span> {{ __('dashboard::dashboard.operations_ui.notifications.dispatch_results') }}
            </h3>
            <div id="notif-results-panel"
                ...
                <div>
                    ...
                    <p>{{ __('dashboard::dashboard.operations_ui.notifications.no_dispatch') }}</p>
                </div>
            </div>
