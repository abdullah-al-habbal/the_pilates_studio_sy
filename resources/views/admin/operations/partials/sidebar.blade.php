...
<aside class="lg:col-span-3 space-y-4">
    <div class="glass-card rounded-2xl p-4">
        <h2 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4 px-2">{{ __('dashboard::dashboard.operations_ui.sidebar.navigation') }}</h2>
        <nav class="space-y-1">
            <button data-tab="clients"
                ...
                <span>{{ __('dashboard::dashboard.operations_ui.sidebar.clients_packages') }}</span>
            </button>
            <button data-tab="store"
                ...
                <span>{{ __('dashboard::dashboard.operations_ui.sidebar.store_inventory') }}</span>
            </button>
            <button data-tab="finance"
                ...
                <span>{{ __('dashboard::dashboard.operations_ui.sidebar.finance_balance') }}</span>
            </button>
            <button data-tab="notifications"
                ...
                <span>{{ __('dashboard::dashboard.operations_ui.sidebar.push_notifications') }}</span>
            </button>
            @if(auth()->user()?->isMainAdmin())
                <button data-tab="approvals"
                    ...
                    <span>{{ __('dashboard::dashboard.operations_ui.sidebar.expense_approvals') }}</span>
                </button>
            @endif
        </nav>
    </div>
