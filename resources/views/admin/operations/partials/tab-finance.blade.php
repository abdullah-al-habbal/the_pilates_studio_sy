...
<div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex flex-wrap gap-2 items-center" id="currency-filters">
        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mr-1">{{ __('dashboard.operations_ui.finance.show_label') }}</span>
        ...
        <button onclick="document.querySelectorAll('.currency-filter-cb').forEach(c=>c.checked=true);OperationsFinance.applyCurrencyFilter()"
                class="text-xs text-primary-600 underline ml-1 hover:text-primary-700">{{ __('dashboard.operations_ui.finance.all_label') }}</button>
        <label class="flex items-center gap-2 ml-4 text-xs font-medium text-slate-600 dark:text-slate-300 cursor-pointer">
            <input type="checkbox" id="convert-to-base" class="rounded border-slate-300"
                   onchange="OperationsFinance.applyCurrencyFilter()">
            {{ __('dashboard.operations_ui.finance.convert_to_base') }} ({{ $defaultCurrency->code }})
        </label>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass-card rounded-2xl p-6 space-y-4">
            <h3 class="text-lg font-bold">{{ __('dashboard.operations_ui.finance.record_expense') }}</h3>
            <form id="expense-form" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1 relative">
                        <label class="text-xs font-semibold text-slate-500 uppercase">{{ __('dashboard.operations_ui.finance.category_label') }}</label>
                        <input type="text" name="category_name" id="category-input" required
                               autocomplete="off"
                               placeholder="{{ __('dashboard.operations_ui.finance.category_placeholder') }}"
                    ...
                    <div class="space-y-1">
                        <label class="text-xs font-semibold text-slate-500 uppercase">{{ __('dashboard.operations_ui.finance.amount_label') }}</label>
                        <input type="number" name="amount" required placeholder="0" step="1" min="0"
                               class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg border-transparent focus:ring-2 focus:ring-primary-500 outline-none">
                    </div>
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-semibold text-slate-500 uppercase">{{ __('dashboard.operations_ui.finance.currency_label') }}</label>
                ...
                <div class="space-y-1">
                    <label class="text-xs font-semibold text-slate-500 uppercase">{{ __('dashboard.operations_ui.finance.notes_label') }}</label>
                ...
                <button type="submit"
                    ...
                    {{ __('dashboard.operations_ui.finance.save_expense') }}
                </button>
            </form>
        </div>

        <div class="glass-card rounded-2xl p-6 space-y-4">
            <h3 class="text-lg font-bold">{{ __('dashboard.operations_ui.finance.expense_breakdown') }}</h3>
