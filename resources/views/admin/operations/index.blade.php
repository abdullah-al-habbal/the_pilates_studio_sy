...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <aside class="lg:col-span-3 space-y-4">
        <div class="glass-card rounded-2xl p-4">
            ...
            <nav class="space-y-1">
                <button data-tab="clients" class="w-full text-left px-4 py-3 rounded-xl transition-all flex items-center gap-3 bg-primary-600 text-white shadow-lg shadow-primary-500/20 active-tab">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                    <span>Clients & Packages</span>
                </button>
                <button data-tab="store" class="w-full text-left px-4 py-3 rounded-xl transition-all flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-800">
                ...
                <button data-tab="finance" class="w-full text-left px-4 py-3 rounded-xl transition-all flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-800">
    ...
    <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div class="flex justify-between items-end gap-4">
            ...
            <div class="relative w-full max-w-sm">
                ...
                <svg class="w-5 h-5 absolute left-3 top-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </div>
        </div>
                ...
                <form id="expense-form" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input type="text" name="category_name" required placeholder="Rent, Electricity..." class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg border-transparent focus:ring-2 focus:ring-primary-500 outline-none">
                        </div>
                        <div class="space-y-1">
                            <label class="text-xs font-semibold text-slate-500 uppercase">Amount (SYP)</label>
                            <input type="number" name="amount" required placeholder="0" class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg border-transparent focus:ring-2 focus:ring-primary-500 outline-none">
                        </div>
                    </div>
                    <div class="space-y-1">
                        <label class="text-xs font-semibold text-slate-500 uppercase">Notes</label>
                        <textarea name="notes" rows="2" class="w-full px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg border-transparent focus:ring-2 focus:ring-primary-500 outline-none"></textarea>
                    </div>
                    <button type="submit" class="w-full bg-slate-900 dark:bg-white dark:text-slate-900 text-white font-bold py-3 rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all">Save Expense</button>
                </form>
            </div>
